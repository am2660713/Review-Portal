...
async function prefillAssignment() {
  ...
  if (!assignmentId) return (status.textContent = "Assignment is missing.");
...
form.addEventListener("submit", async (e) => {
  ...
  try { await api("/api/reviewer/review", { method: "POST", body: JSON.stringify(payload) }); status.textContent = "Submitted successfully."; setTimeout(() => window.location.href = "/manager.html", 1000); }
